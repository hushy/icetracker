...
const presetKeys=['periodMinutes','periods','shiftEnabled','shiftSeconds','endHorn','volume','hornSound','breakMinutes','autoPauseOnGoalPenalty'];
export const presetSettings=settings=>Object.fromEntries(presetKeys.map(key=>[key,settings[key]]));
export function undoGame(snapshot){return pauseClocks(structuredClone(snapshot));}
export function editMatchEvent(game,id,patch){
 const old=game.events.find(event=>event.id===id);if(!old)return game;
 const edited={...old,...patch,id:old.id,type:old.type,editedAt:Date.now()};
 ...
 return {...game,events:game.events.map(event=>event.id===id?edited:event),goals:game.goals.map(goal=>old.type==='Goal'&&goal.id===goalId?{...goal,scorer:edited.scorer||'',assists:edited.assists||[],period:edited.period,remainingMs:edited.remainingMs}:goal)};
}
