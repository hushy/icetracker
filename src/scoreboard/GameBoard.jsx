...
export default function GameBoard({ game, actions, toolbar, hornFlash = '', disconnected = false }) {
  ...
  return <div className={`scoreboard integrated-board compact-board ${hornFlash?'horn-flash':''}`} style={game.settings.background?{backgroundImage:`linear-gradient(rgba(6,10,35,.78),rgba(6,10,35,.88)), url("${game.settings.background}")`}:undefined}>
    ...
    <div className="board-grid">{teamPanel('home')}<section className="clock-panel" aria-label={t('Game clock')}>
      ...
      <div className="clock-status">{!running&&!ended&&<div className="pause-warning" role="status">{t('Clock paused')}</div>}{ended&&<div className="period-ended" role="status">{t(active?'Countdown complete':finalPeriod?'GAME COMPLETE':'PERIOD COMPLETE')}</div>}</div>
      ...
      <PenaltyAnnouncement game={game} disconnected={disconnected}/>
