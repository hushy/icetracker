...
The main **Scoreboard** keeps scores, penalties, clock and period controls together. **Match sheet** contains the event journal and editing/export actions. The simplified header contains a compact FR/EN language switch, new game, undo, public screen and **Configuration**. Configuration is the single settings page — teams, clock, game options, horn and junior shifts, saved configurations and device options — and play continues while it is open; changes apply when you save. Fullscreen is a public-screen control only.

- **New game** opens a chooser: select a saved clock/horn configuration and confirm **Start new game**, or choose **Configure game** to set up the next match. The score and match sheet reset only on final confirmation; the new clock stays paused. Teams/artwork are retained when using a saved configuration. Create configurations under **Configuration → Saved configuration**.
- Tap either team name on the operator board to edit its name, club, youth category and optional logo without stopping play. Set period length/count in **Configuration**; team setup, game options, horn/shifts, saved configurations and device options are collapsed there for an existing game.
...
- Fixed Français Volants navy/white theme and uploaded team logos up to 1 MB each. Theme and background-image controls are gone: a scoreboard read from the stands does not need them.
- One horn, the industrial air horn, at a fixed 70%. The sound picker, its samples and the volume slider are removed; **Test horn** in Configuration still checks the device before a game.
- Game time, penalties and junior shifts advance together. Pause/resume with the large control or Space outside a control/dialog. On the operator screen, running countdowns show a green LIVE indicator and paused countdowns show their wall-clock pause duration. The public screen has only one small glyph: a green play when it runs and a grey pause when it is stopped. The red operator warning becomes brighter and pulses faster after 15, 45 and 120 seconds (bounded at 0.75 seconds per pulse). Reduced-motion mode keeps a steady warning. Resume clears the pause duration; the next pause starts at zero.
- Use the labelled **Edit** control above the clock to correct the currently visible game, break or timeout countdown. The form shows the live value while it is open; saving only changed fields applies them to the current synced clock, preserving its running/paused state and all penalty and shift timers. Goal and penalty entry do not pause by default. **Auto pause on goal/penalty** in setup is opt-in. Confirming a penalty starts it if play is running, unless it is marked waiting.
...
- Period end freezes clocks and can sound a horn. Next period retains remaining penalties and resets game/shift clocks paused; scores carry over unless per-period scoring is enabled. Overtime duration follows the period setting unless manually edited.
...
Goal and penalty occurrence times are captured when the operator opens the form, independently of confirmation time. A non-waiting penalty timer starts on confirmation. **Served by** is optional and separate from the offending player. Report times are counted inside their own period, next to the period number; they are never added together across periods. A penalty starting and ending in different periods shows the period beside its start or end time. Each period's configured length is retained so clock corrections stay consistent. Missing historical period lengths stay unknown rather than being guessed.
...
**Configuration → Saved configuration** stores named period, break, shift and game-option settings locally. Presets hold no rosters, images or live game state. Applying one fills the configuration form and changes nothing until you save, so one panel serves both a running match and the new-game draft. Sound is never stored: a preset written by an older version cannot bring back a horn choice or a volume.
...
On a screen 701 px wide or more, the public display puts the clock in a full-width header — period on the left, clock centred, state glyph on the right — and gives the whole width below it to the two teams, so names, scores and penalties are sized for a rink wall instead of a third column. Goal and penalty announcements span that width under the clock, and the clock and scores shrink for their duration so an active penalty is never pushed off screen. The operator board keeps its three-column layout.
...
Entry errors use the labelled **Erreur / Entry error** action on a penalty. Confirmation removes that penalty and all its linked events without recording a release, showing an announcement or pausing—even when automatic pause is enabled. The normal **Release** action is for an actual penalty ending and remains recorded and announced. Changing the per-period score option takes effect at the next period; completed-period goals remain available in the report.
